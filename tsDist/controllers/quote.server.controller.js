 ...
 * Module dependencies.
 */
var mongoose = require('mongoose'), passport = require('passport'), _ = require('lodash'), async = require('async'), yql = require('yql-node'), config = require('../../config/config'), errorHandler = require('./errors.server.controller'), stockPositionsCtrl = require('./stockpositions/sp.base.server.controller'), Exchanges = require('../enums/exchanges.server.enums'), Quote = mongoose.model('Quote'), Fundamentals = mongoose.model('Fundamentals'), Performance = mongoose.model('Performance');
/**
 * Create a Quote
 */
...
function updateReferences(response, symbol, yahooSymbol, res) {
    var query = { 'Symbol': symbol }, options = { 'upsert': true, 'new': true, 'setDefaultOnInsert': true }, quoteResponse = JSON.parse(response).query.results.quote;
...
//# sourceMappingURL=quote.server.controller.js.map