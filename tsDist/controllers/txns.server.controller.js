 ...
 * Module dependencies.
 */
var mongoose = require('mongoose'), errorHandler = require('./errors.server.controller'), Txn = mongoose.model('Txn'), _ = require('lodash');
/**
 * Create a Txn
 */
...
exports.txnByID = function (req, res, next, id) {
    Txn.findById(id).populate('user', 'displayName').exec(function (err, txn) {
        if (err)
            return next(err);
        if (!txn)
            return next(new Error('Failed to load Txn ' + id));
        req.txn = txn;
        next();
    });
};
...
//# sourceMappingURL=txns.server.controller.js.map