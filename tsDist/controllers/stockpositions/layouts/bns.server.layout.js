 ...
 * Module dependencies.
 */
var mongoose = require('mongoose'), StockpositionModel = mongoose.model('Stockposition'), Txn = mongoose.model('Txn'), GainLoss = mongoose.model('GainLoss'), Income = mongoose.model('Income'), TxnTypes = require('../../../enums/txntypes.server.enums'), csvParse = require('csv-parse'), errorHandler = require('../../errors.server.controller'), Stockposition = require('../../stockpositions.server.controller'), startPosition = 4, endPosition = 10, logicalRecordLength = 11, records = [{}];
class BaseLayout {
...
function importGainLoss(req, res) {
    csvParse(req.body.file, { delimiter: ',', rowDelimiter: '\n', relax_column_count: 'true' }, function (err, output) {
        ...
        var options = { 'upsert': true, 'new': true, 'setDefaultOnInsert': true }, errorMessage;
        for (var i = 1; i <= output.length - 1; i++) {
            var record = output[i], gainLoss = new GainLoss({
...
function importStockPosition(req, res) {
    var layout = req.body.layout, fileData = req.body.file;
...
function importTrades(req, res) {
    csvParse(req.body.file, { delimiter: ',', rowDelimiter: '\n', relax_column_count: 'true' }, function (err, output) {
        ...
        for (var i = 1; i <= output.length - 1; i++) {
            var record = output[i];
            var description = String(record[0]).trim(), symbol = String(record[1]), settlementDate = record[2], shares = Math.abs(Number(record[6])), price = Number(record[8]), settleAmount = Math.abs(Number(record[9]).toFixed(2)), accountType = 'rsp';
            //Do dividends
...
//# sourceMappingURL=bns.server.layout.js.map